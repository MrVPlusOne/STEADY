...
@kwdef(struct ParticleFilterSampler <: PosteriorSampler
    n_particles::Int
    n_trajs::Int
end)
...
function sample_posterior_parallel(
    ...
    results = parallel_map(1:n_runs, nothing; n_threads) do _, i
        r = sample_posterior(
            sampler, systems[i], obs_data_list[i], states[i]; showprogress=false
        )
        next!(progress)
        r
    end
...
@kwdef(struct ParticleGibbsSampler <: PosteriorSampler
    n_particles::Int
    n_jumps::Int
    thining::Int
...
function sample_posterior(
    ...
    if isempty(state)
        ...
        (; trajectories) = filter_smoother(
            system,
            obs_data;
            use_auxiliary_proposal=false,
            pf.n_particles,
            n_trajs=1,
            pf.resample_threshold,
            showprogress,
        )
    ...
    kernel = PGAS_kernel(
        system, obs_data; showprogress=false, pf.n_particles, pf.resample_threshold
    )
    ...
    showprogress &&
        (progress = Progress(pf.n_jumps; desc="sample_posterior", output=stdout))
