...
function map_synthesis(
    ...
    progress = Progress(length(all_comps); desc="evaluating", showspeed=true, output=stdout)

    eval_results = parallel_map(
        evaluate_prog, all_comps, ctx; progress, n_threads, use_distributed
    )
    ...
    first_opt_time, mean_opt_time = (f.(all_times) |> to_measurement for f in (first, mean))
    ...
    opt_success_rate = (
        succeeded |>
        Map(r -> length(r.optimize_times) / evals_per_program) |>
        to_measurement
    )
...
function evaluate_prog(ctx, comps, f_x′′)
    ...
    MAP_est = (sol.params, sol.others, states=sol.traj, logp=sol.logp)
