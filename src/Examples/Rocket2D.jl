...
landmark_dist(n) = begin
    ...
    DistrIterator([SMvUniform((-40.0, 40.0), (-20.0, 10.0)) for _ in 1:n])
end
...
variable_data(n_landmarks, (; pos, θ), x₀_σ=0.01) = VariableData(;
    ...
    others=OrderedDict(:landmarks => landmark_dist(n_landmarks)),
)
...
function acceleration_f((;
    ...
    # speed = norm_R2(pos′)
    thrust_force = rotate2d(θ, @SVector [0.0, thrust])
...
ground_truth_sketch() = begin
    ...
    DynamicsSketch(Var[], combine)
end

simple_sketch() =
    let
        combine(inputs, (; ext_force, ext_moment)) =
            let
        ...
        DynamicsSketch(
            [Var(:ext_force, ℝ2, PUnits.Force), Var(:ext_moment, ℝ, PUnits.Torque)], combine
        )
    end
...
generate_data(x₀, x′₀, params, others, times; noise_scale, target_pos) = begin
    ...
    SEDL.generate_data(
        ...
        controller=(s, o) -> controller(s, o; weight, target_pos),
        allow_state_access=true,
    )
end


data_likelihood(observations; noise_scale, data_thining::Int, check_dual=false) =
    (states, others) -> let
        dl(i) =
            let
                s = states[i]
                ...
                if check_dual && SEDL.is_bad_dual(v)
                    ...
                    @info "data_likelihood" distr = observation_dist(
                        s, s_prev, others; noise_scale
                    )
