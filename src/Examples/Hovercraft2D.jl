...
variables(::HovercraftScenario) = variable_tuple(
    ...
    # params
    :mass => ℝ(PUnits.Mass),
    :drag_x => ℝ(PUnits.Force / PUnits.Speed^2),
    :drag_y => ℝ(PUnits.Force / PUnits.Speed^2),
...
function variable_data(scenario::HovercraftScenario, x0) 
...
function observation_dist(sce::HovercraftScenario)
    ...
    state -> begin
        lmr=landmark_readings(state, landmarks; bearing_only=false,
            sensor_range=7.0, σ_range = noise_scale * 1.2, σ_bearing = noise_scale * 5°)
...
function dynamics_sketch(sce::HovercraftScenario)
    ...
    params = OrderedDict{Var, GDistr}(
        ...
        vars.drag_x => PertBeta(0.0, 0.04, 0.15),
        vars.drag_y => PertBeta(0.0, 0.05, 0.15),
        vars.rot_mass => InverseGamma(5, 5),
        vars.rot_drag => PertBeta(0.0, 0.1, 0.2),
    ...
    outputs_to_state_dist((; f_x, f_x, f_θ), others, Δt) = begin
        # TODO: think about how to use leap-frog with Brownian disturbance

        (; pos, vel, θ, ω, ul, ur, mass, rot_mass, σ_v, σ_ω) = others

        loc_ax = (ul + ur - f_x) / mass
        loc_ay = -f_x / mass
        der_ω = (ur - ul - f_θ) / rot_mass
