...
function car_motion_model(
    (; drag, mass); f_x′′=Car1D.acceleration_f, σ=(σ_pos=0.1, σ_pos′=0.2)
)
    (state, ctrl, Δt) -> begin
        (pos, pos′) = state
        f = ctrl[1]
...
pfs_result = @time filter_smoother(bad_system, obs_data, 10000; resample_threshold=0.5)
plot_particles(pfs_result[1], times, "filter-smoother", ex_data.states) |> display

ffbs_result = @time ffbs_smoother(
    bad_system, obs_data; n_particles=10_000, n_trajs=200, resample_threshold=0.5
)
...
map_params = @time fit_dynamics_params(
    car_motion_model,
    subsample(ffbs_result.particles, 200),
    subsample(ffbs_result.log_weights, 200),
    x0_dist,
    params_dist,
    obs_data,
    (; mass=1.0, drag=0.5);
...
function check_performance_variance(
    system, obs_data; n_particles, n_trajs, use_ffbs, repeats=50
)
...
if !skip_variance_checking
    check_performance_variance(
        bad_system, obs_data; n_particles=10_000, n_trajs=200, use_ffbs=false
    )
    check_performance_variance(
        bad_system, obs_data; n_particles=10_000, n_trajs=200, use_ffbs=true
    )
end
##-----------------------------------------------------------
...
rand_inputs = [
    merge((pos=randn(), pos′=randn(), f=2randn()), rand(params_dist)) for _ in 1:200
]
...
optim_options = Optim.Options(;
    f_abstol=1e-4, iterations=100, outer_iterations=40, time_limit=2.0
)
...
# run full synthesis
iter_result =
    let senum = synthesis_enumeration(vdata, good_sketch, shape_env, comp_env, 7; pruner)
...
plot(
    iter_result.logp_history; xlabel="iterations", title="log p(observations)", legend=false
) |> display
