...
ffbs_result = @time filter_smoother(true_system, obs_data, n_particles=1000_000, n_trajs=100)
ffbs_result.trajectories
let plt = plot()
    Car2D.plot_trajectories!(ffbs_result.trajectories, "FFBS", ex_data.states)
    @unzip xs, ys = landmarks
    scatter!(xs, ys, label="Landmarks")
    ...
    @time fit_dynamics_iterative(senum, obs_data, comps_guess, params_guess;
        ...
        iteration_callback = (iter, particles) -> let
            if mod1(iter, 5) == 1
                ...
                Car2D.plot_trajectories!(particles, "posterior", ex_data.states)
                display("image/png", p)
            end
        end
    )
